...
static int lfs2_bd_read(lfs2_t *lfs2, const lfs2_cache_t *pcache,
		lfs2_cache_t *rcache, lfs2_size_t hint, lfs2_block_t block,
	...
	while (size > 0) {
		...
		rcache->size = lfs2_min(
				lfs2_min(lfs2_alignup(off + hint, lfs2->cfg->read_size),
						lfs2->cfg->block_size) - rcache->off,
				lfs2->cfg->cache_size);
...
enum {
	LFS2_CMP_EQ = 0, LFS2_CMP_LT = 1, LFS2_CMP_GT = 2,
};
...
static int lfs2_bd_cmp(lfs2_t *lfs2, const lfs2_cache_t *pcache,
	...
	for (lfs2_off_t i = 0; i < size; i++) {
		uint8_t dat;
		int err = lfs2_bd_read(lfs2, pcache, rcache, hint - i, block, off + i,
				&dat, 1);
		if (err) {
			return err;
		}
...
static int lfs2_bd_flush(lfs2_t *lfs2, lfs2_cache_t *pcache,
	...
	if (pcache->block != LFS2_BLOCK_NULL && pcache->block != LFS2_BLOCK_INLINE) {
		...
		if (validate) {
			...
			NULL, rcache, diff, pcache->block, pcache->off, pcache->buffer,
					diff);
			if (res < 0) {
				return res;
			}
...
static int lfs2_bd_prog(lfs2_t *lfs2, lfs2_cache_t *pcache,
		lfs2_cache_t *rcache, bool validate, lfs2_block_t block, lfs2_off_t off,
...
static int lfs2_dir_compact(lfs2_t *lfs2, lfs2_mdir_t *dir,
		const struct lfs2_mattr *attrs, int attrcount, lfs2_mdir_t *source,
...
int lfs2_fs_traverseraw(lfs2_t *lfs2, int (*cb)(void *data, lfs2_block_t block),
		void *data,
		bool includeorphans);
...
static lfs2_stag_t lfs2_dir_getslice(lfs2_t *lfs2, const lfs2_mdir_t *dir,
		lfs2_tag_t gmask, lfs2_tag_t gtag, lfs2_off_t goff, void *gbuffer,
		lfs2_size_t gsize) {
	...
	while (off >= sizeof(lfs2_tag_t) + lfs2_tag_dsize(ntag)) {
		...
		NULL, &lfs2->rcache, sizeof(ntag), dir->pair[0], off, &ntag,
				sizeof(ntag));
		if (err) {
			return err;
		}
		...
		if ((gmask & tag) == (gmask & (gtag - gdiff))) {
			...
			NULL, &lfs2->rcache, diff, dir->pair[0], off + sizeof(tag) + goff,
					gbuffer, diff);
			if (err) {
				return err;
			}
...
static int lfs2_dir_getread(lfs2_t *lfs2, const lfs2_mdir_t *dir,
		...
		lfs2_tag_t gmask, lfs2_tag_t gtag, lfs2_off_t off, void *buffer,
		lfs2_size_t size) {
	uint8_t *data = (uint8_t *) buffer;
...
static int lfs2_dir_traverse_filter(void *p, lfs2_tag_t tag, const void *buffer) {
	lfs2_tag_t *filtertag = (lfs2_tag_t *) p;
	(void) buffer;

	// which mask depends on unique bit in tag structure
	uint32_t mask =
			(tag & LFS2_MKTAG(0x100, 0, 0)) ?
					LFS2_MKTAG(0x7ff, 0x3ff, 0) : LFS2_MKTAG(0x700, 0x3ff, 0);
...
static int lfs2_dir_traverse(lfs2_t *lfs2, const lfs2_mdir_t *dir,
		lfs2_off_t off, lfs2_tag_t ptag, const struct lfs2_mattr *attrs,
		int attrcount, lfs2_tag_t tmask, lfs2_tag_t ttag, uint16_t begin,
		uint16_t end, int16_t diff,
	...
	while (true) {
		...
		if (off + lfs2_tag_dsize(ptag) < dir->off) {
			...
			int err = lfs2_bd_read(lfs2,
			NULL, &lfs2->rcache, sizeof(tag), dir->pair[0], off, &tag,
					sizeof(tag));
			if (err) {
				return err;
			}
		...
		if (lfs2_tag_id(tmask) != 0) {
			...
			int filter = lfs2_dir_traverse(lfs2, dir, off, ptag, attrs,
					attrcount, 0, 0, 0, 0, 0, lfs2_dir_traverse_filter, &tag);
			if (filter < 0) {
				return filter;
			}
		...
		if (lfs2_tag_type3(tag) == LFS2_FROM_NOOP) {
			...
			int err = lfs2_dir_traverse(lfs2, (const lfs2_mdir_t *) buffer, 0, 0xffffffff, NULL, 0,
					LFS2_MKTAG(0x600, 0x3ff, 0),
					LFS2_MKTAG(LFS2_TYPE_STRUCT, 0, 0), fromid, fromid + 1,
			...
			for (unsigned i = 0; i < lfs2_tag_size(tag); i++) {
				const struct lfs2_attr *a = (const struct lfs2_attr *) buffer;
				int err = cb(data,
...
static lfs2_stag_t lfs2_dir_fetchmatch(lfs2_t *lfs2, lfs2_mdir_t *dir,
		const lfs2_block_t pair[2], lfs2_tag_t fmask, lfs2_tag_t ftag,
		uint16_t *id, int (*cb)(void *data, lfs2_tag_t tag, const void *buffer),
		void *data) {
	...
	if (pair[0] >= lfs2->cfg->block_count
			|| pair[1] >= lfs2->cfg->block_count) {
		return LFS2_ERR_CORRUPT;
	}
	...
	for (int i = 0; i < 2; i++) {
		int err = lfs2_bd_read(lfs2,
		NULL, &lfs2->rcache, sizeof(revs[i]), pair[i], 0, &revs[i],
				sizeof(revs[i]));
	...
	for (int i = 0; i < 2; i++) {
		...
		while (true) {
			...
			off += lfs2_tag_dsize(ptag);
			int err = lfs2_bd_read(lfs2,
			NULL, &lfs2->rcache, lfs2->cfg->block_size, dir->pair[0], off, &tag,
					sizeof(tag));
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
					// can't continue?
			...
			if (lfs2_tag_type1(tag) == LFS2_TYPE_CRC) {
				...
				uint32_t dcrc;
				err = lfs2_bd_read(lfs2,
				NULL, &lfs2->rcache, lfs2->cfg->block_size, dir->pair[0],
...
//				int res = cb(data, tag, &(struct lfs2_diskoff ) { dir->pair[0], off + sizeof(tag) });
				struct lfs2_diskoff buffer;
				buffer.block = dir->pair[0];
				buffer.off = off + sizeof(tag);
				int res = cb(data, tag, &buffer);
	...
	LFS2_ERROR("Corrupted dir pair at {0x%"PRIx32", 0x%"PRIx32"}", dir->pair[0],
			dir->pair[1]);
	return LFS2_ERR_CORRUPT;
}
...
static int lfs2_dir_find_match(void *data, lfs2_tag_t tag, const void *buffer) {
	struct lfs2_dir_find_match *name = (struct lfs2_dir_find_match *) data;
	lfs2_t *lfs2 = name->lfs2;
	const struct lfs2_diskoff *disk = (const struct lfs2_diskoff *) buffer;

	// compare with disk
	lfs2_size_t diff = lfs2_min(name->size, lfs2_tag_size(tag));
	int res = lfs2_bd_cmp(lfs2,
	NULL, &lfs2->rcache, diff, disk->block, disk->off, name->name, diff);
	if (res != LFS2_CMP_EQ) {
		return res;
	}
	...
	while (true) {
		...
		*path = name;
		...
		if (lfs2_tag_id(tag) != 0x3ff) {
			lfs2_stag_t res = lfs2_dir_get(lfs2, dir,
					LFS2_MKTAG(0x700, 0x3ff, 0),
					LFS2_MKTAG(LFS2_TYPE_STRUCT, lfs2_tag_id(tag), 8),
					dir->tail);
			if (res < 0) {
				return res;
			}
...
static int lfs2_dir_commitprog(lfs2_t *lfs2, struct lfs2_commit *commit,
		const void *buffer, lfs2_size_t size) {
	int err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, false,
			commit->block, commit->off, (const uint8_t*) buffer, size);
	if (err) {
		return err;
	}
...
static int lfs2_dir_commitattr(lfs2_t *lfs2, struct lfs2_commit *commit,
	...
	if (!(tag & 0x80000000)) {
		...
		for (lfs2_off_t i = 0; i < dsize - sizeof(tag); i++) {
			...
			uint8_t dat;
			err = lfs2_bd_read(lfs2,
			NULL, &lfs2->rcache, dsize - sizeof(tag) - i, disk->block,
					disk->off + i, &dat, 1);
			if (err) {
				return err;
			}
...
static int lfs2_dir_commitcrc(lfs2_t *lfs2, struct lfs2_commit *commit) {
	...
	while (commit->off < end) {
		...
		int err = lfs2_bd_read(lfs2,
		NULL, &lfs2->rcache, sizeof(tag), commit->block, noff, &tag,
				sizeof(tag));
		if (err && err != LFS2_ERR_CORRUPT) {
			return err;
		}
		...
		err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, false,
	...
	while (off < end) {
		...
		for (lfs2_off_t i = off; i < noff + sizeof(uint32_t); i++) {
			...
			uint8_t dat;
			err = lfs2_bd_read(lfs2,
			NULL, &lfs2->rcache, noff + sizeof(uint32_t) - i, commit->block, i,
					&dat, 1);
			if (err) {
				return err;
			}
...
static int lfs2_dir_alloc(lfs2_t *lfs2, lfs2_mdir_t *dir) {
	...
	int err = lfs2_bd_read(lfs2,
	NULL, &lfs2->rcache, sizeof(dir->rev), dir->pair[0], 0, &dir->rev,
			sizeof(dir->rev));
	dir->rev = lfs2_fromle32(dir->rev);
	if (err && err != LFS2_ERR_CORRUPT) {
		return err;
...
static int lfs2_dir_commit_size(void *p, lfs2_tag_t tag, const void *buffer) {
	lfs2_size_t *size = (lfs2_size_t *) p;
	(void) buffer;
...
static int lfs2_dir_commit_commit(void *p, lfs2_tag_t tag, const void *buffer) {
	struct lfs2_dir_commit_commit *commit = (struct lfs2_dir_commit_commit *) p;
...
static int lfs2_dir_compact(lfs2_t *lfs2, lfs2_mdir_t *dir,
	...
	while (end - begin > 1) {
		// find size
		lfs2_size_t size = 0;
		int err = lfs2_dir_traverse(lfs2, source, 0, 0xffffffff, attrs,
				attrcount, LFS2_MKTAG(0x400, 0x3ff, 0),
				LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end, -begin,
		...
		if (end - begin < 0xff
				&& size
						<= lfs2_min(lfs2->cfg->block_size - 36,
		...
		err = lfs2_dir_split(lfs2, dir, attrs, attrcount, source, begin + split,
				end);
		if (err) {
	...
	if (lfs2->cfg->block_cycles > 0
		...
		if (lfs2_pair_cmp(dir->pair, (const lfs2_block_t[2] ) { 0, 1 }) == 0) {
			...
			if ((lfs2_size_t) res < lfs2->cfg->block_count / 2) {
				...
				int err = lfs2_dir_split(lfs2, dir, attrs, attrcount, source,
						begin, end);
	...
	while (true) {
		{
			...
			// write out header
			dir->rev = lfs2_tole32(dir->rev);
			err = lfs2_dir_commitprog(lfs2, &commit, &dir->rev,
					sizeof(dir->rev));
			dir->rev = lfs2_fromle32(dir->rev);
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
			...
			struct lfs2_dir_commit_commit data =  {lfs2, &commit };
			err = lfs2_dir_traverse(lfs2, source, 0, 0xffffffff, attrs,
					attrcount, LFS2_MKTAG(0x400, 0x3ff, 0),
					LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end, -begin,
					lfs2_dir_commit_commit, &data);
//			err = lfs2_dir_traverse(lfs2, source, 0, 0xffffffff, attrs,
//					attrcount, LFS2_MKTAG(0x400, 0x3ff, 0),
//					LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end, -begin,
//					lfs2_dir_commit_commit, &(struct lfs2_dir_commit_commit ) {lfs2, &commit });
			...
			if (!lfs2_gstate_iszero(&delta)) {
				...
				err = lfs2_dir_commitattr(lfs2, &commit,
						LFS2_MKTAG(LFS2_TYPE_MOVESTATE, 0x3ff, sizeof(delta)),
						&delta);
				if (err) {
					if (err == LFS2_ERR_CORRUPT) {
						goto relocate;
	...
	if (relocated) {
		...
		LFS2_DEBUG(
				"Relocating {0x%"PRIx32", 0x%"PRIx32"} " "-> {0x%"PRIx32", 0x%"PRIx32"}",
...
static int lfs2_dir_commit(lfs2_t *lfs2, lfs2_mdir_t *dir,
	...
	if (dir->erased || dir->count >= 0xff) {
		...
		struct lfs2_dir_commit_commit data =  { lfs2, &commit };
		int err = lfs2_dir_traverse(lfs2, dir, dir->off, dir->etag, attrs,
				attrcount, 0, 0, 0, 0, 0, lfs2_dir_commit_commit,
				&data);
...
//				&(struct lfs2_dir_commit_commit ) { lfs2, &commit });
		...
		int err = lfs2_dir_compact(lfs2, dir, attrs, attrcount, dir, 0,
				dir->count);
		if (err) {
			*dir = olddir;
			return err;
...
int lfs2_mkdir(lfs2_t *lfs2, const char *path) {
	...
	struct lfs2_mattr attr;
	attr.tag =	LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8);
	attr.buffer = pred.tail;
	err = lfs2_dir_commit(lfs2, &dir, &attr, sizeof(attr) / sizeof(struct lfs2_mattr));
//	err = lfs2_dir_commit(lfs2, &dir,
//				LFS2_MKATTRS({
...
//			);
	...
	if (cwd.m.split) {
		...
		struct lfs2_mattr attrs;
		attrs.tag = LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8);
		attrs.buffer = dir.pair;
		err = lfs2_dir_commit(lfs2, &pred, &attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
...
//							  );
	...
	struct lfs2_mattr attrs[4];
	attrs[0].tag = LFS2_MKTAG(LFS2_TYPE_CREATE, id, 0);
	attrs[0].buffer = NULL;
	attrs[1].tag = LFS2_MKTAG(LFS2_TYPE_DIR, id, nlen);
	attrs[1].buffer = path;
	attrs[2].tag = LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, id, 8);
	attrs[2].buffer = dir.pair;
	attrs[3].tag = LFS2_MKTAG_IF(!cwd.m.split, LFS2_TYPE_SOFTTAIL, 0x3ff, 8);
	attrs[3].buffer = dir.pair;
	err = lfs2_dir_commit(lfs2, &cwd.m, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr));

//	err = lfs2_dir_commit(lfs2, &cwd.m,
//			LFS2_MKATTRS(
//						{LFS2_MKTAG(LFS2_TYPE_CREATE, id, 0), NULL},
//						{LFS2_MKTAG(LFS2_TYPE_DIR, id, nlen), path},
//						{LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, id, 8), dir.pair},
//						{LFS2_MKTAG_IF(!cwd.m.split, LFS2_TYPE_SOFTTAIL, 0x3ff, 8),	dir.pair}
//					)
//			);
...
int lfs2_dir_open(lfs2_t *lfs2, lfs2_dir_t *dir, const char *path) {
	LFS2_TRACE("lfs2_dir_open(%p, %p, \"%s\")", (void* )lfs2, (void* )dir,
			path);
	...
	if (lfs2_tag_id(tag) == 0x3ff) {
		...
		lfs2_stag_t res = lfs2_dir_get(lfs2, &dir->m,
				LFS2_MKTAG(0x700, 0x3ff, 0),
...
static int lfs2_ctz_find(lfs2_t *lfs2, const lfs2_cache_t *pcache,
		lfs2_cache_t *rcache, lfs2_block_t head, lfs2_size_t size,
	...
	lfs2_off_t off_index =  { size - 1 };
	lfs2_off_t current = lfs2_ctz_index(lfs2, &off_index);
//	lfs2_off_t current = lfs2_ctz_index(lfs2, &(lfs2_off_t ) { size - 1 });

	lfs2_off_t target = lfs2_ctz_index(lfs2, &pos);

	while (current > target) {
		lfs2_size_t skip = lfs2_min(lfs2_npw2(current - target + 1) - 1,
				lfs2_ctz(current));

		int err = lfs2_bd_read(lfs2, pcache, rcache, sizeof(head), head,
				4 * skip, &head, sizeof(head));
...

		{
			...
			if (noff != lfs2->cfg->block_size) {
				for (lfs2_off_t i = 0; i < noff; i++) {
					uint8_t data;
					err = lfs2_bd_read(lfs2,
					NULL, rcache, noff - i, head, i, &data, 1);
					if (err) {
						return err;
					}

					err = lfs2_bd_prog(lfs2, pcache, rcache, true, nblock, i,
							&data, 1);
					if (err) {
						if (err == LFS2_ERR_CORRUPT) {
							goto relocate;
			...
			for (lfs2_off_t i = 0; i < skips; i++) {
				...
				err = lfs2_bd_prog(lfs2, pcache, rcache, true, nblock, 4 * i,
						&nhead, 4);
				...
				if (i != skips - 1) {
					err = lfs2_bd_read(lfs2,
					NULL, rcache, sizeof(nhead), nhead, 4 * i, &nhead,
							sizeof(nhead));
					nhead = lfs2_fromle32(nhead);
					if (err) {
						return err;
...
static int lfs2_ctz_traverse(lfs2_t *lfs2, const lfs2_cache_t *pcache,
	...
	lfs2_off_t off_index = { size - 1 };
	lfs2_off_t index = lfs2_ctz_index(lfs2, &off_index);
...
int lfs2_file_opencfg(lfs2_t *lfs2, lfs2_file_t *file, const char *path,
	...
	LFS2_TRACE(
			"lfs2_file_opencfg(%p, %p, \"%s\", %x, %p {" ".buffer=%p, .attrs=%p, .attr_count=%"PRIu32"})",
	...
	if (tag == LFS2_ERR_NOENT) {
		...
		err = lfs2_dir_commit(lfs2, &file->m, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr));

//		err = lfs2_dir_commit(lfs2, &file->m,
//				LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_CREATE, file->id, 0)},
//						{LFS2_MKTAG(LFS2_TYPE_REG, file->id, nlen), path},
...
//
	...
	if (file->cfg->buffer) {
		...
		file->cache.buffer = (uint8_t *) lfs2_malloc(lfs2->cfg->cache_size);
		...
		for (lfs2_off_t i = 0; i < file->off; i++) {
			...
			if (file->flags & LFS2_F_INLINE) {
				err = lfs2_dir_getread(lfs2, &file->m,
						...
						LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, file->id, 0), i,
						&data, 1);
				if (err) {
					return err;
				}
			} else {
				err = lfs2_bd_read(lfs2, &file->cache, &lfs2->rcache,
			...
			err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, true, nblock,
					i, &data, 1);
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
					goto relocate;
...
static int lfs2_file_flush(lfs2_t *lfs2, lfs2_file_t *file) {
	...
	if (file->flags & LFS2_F_WRITING) {
		...
		if (!(file->flags & LFS2_F_INLINE)) {
			...
			orig.flags = LFS2_O_RDONLY | LFS2_F_OPENED;
			orig.pos = file->pos;
			orig.cache = lfs2->rcache;

//			lfs2_file_t orig = { .ctz.head = file->ctz.head, .ctz.size =
//					file->ctz.size, .flags = LFS2_O_RDONLY | LFS2_F_OPENED,
//					.pos = file->pos, .cache = lfs2->rcache, };
			...
			while (true) {
				int err = lfs2_bd_flush(lfs2, &file->cache, &lfs2->rcache,
						true);
				if (err) {
					if (err == LFS2_ERR_CORRUPT) {
						goto relocate;
...
int lfs2_file_sync(lfs2_t *lfs2, lfs2_file_t *file) {
	...
	if ((file->flags & LFS2_F_DIRTY) && !lfs2_pair_isnull(file->m.pair)) {
		...
		struct lfs2_mattr attrs [] = {
					...
					{LFS2_MKTAG(LFS2_FROM_USERATTRS, file->id, file->cfg->attr_count), file->cfg->attrs}
				};
		err = lfs2_dir_commit(lfs2, &file->m, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
	...
	while (nsize > 0) {
		...
		if (!(file->flags & LFS2_F_READING)
			...
			if (!(file->flags & LFS2_F_INLINE)) {
				int err = lfs2_ctz_find(lfs2, NULL, &file->cache,
						file->ctz.head, file->ctz.size, file->pos, &file->block,
						&file->off);
				if (err) {
					LFS2_TRACE("lfs2_file_read -> %d", err);
					return err;
		...
		if (file->flags & LFS2_F_INLINE) {
			...
			NULL, &file->cache, lfs2->cfg->block_size,
					LFS2_MKTAG(0xfff, 0x1ff, 0),
					LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, file->id, 0), file->off,
					data, diff);
			if (err) {
				LFS2_TRACE("lfs2_file_read -> %d", err);
				return err;
			...
			int err = lfs2_bd_read(lfs2,
			NULL, &file->cache, lfs2->cfg->block_size, file->block, file->off,
					data, diff);
			if (err) {
				LFS2_TRACE("lfs2_file_read -> %d", err);
				return err;
...
lfs2_ssize_t lfs2_file_write(lfs2_t *lfs2, lfs2_file_t *file,
	...
	if (!(file->flags & LFS2_F_WRITING) && file->pos > file->ctz.size) {
		...
		while (file->pos < pos) {
			...
			uint8_t buffer = { 0 };
			lfs2_ssize_t res = lfs2_file_write(lfs2, file, &buffer, 1);
//			lfs2_ssize_t res = lfs2_file_write(lfs2, file, &(uint8_t ) { 0 }, 1);
	...
	while (nsize > 0) {
		...
		if (!(file->flags & LFS2_F_WRITING)
			...
			if (!(file->flags & LFS2_F_INLINE)) {
				...
				int err = lfs2_ctz_extend(lfs2, &file->cache, &lfs2->rcache,
						file->block, file->pos, &file->block, &file->off);
				if (err) {
					file->flags |= LFS2_F_ERRED;
					LFS2_TRACE("lfs2_file_write -> %d", err);
					return err;
		...
		while (true) {
			...
			break;
			relocate: err = lfs2_file_relocate(lfs2, file);
			if (err) {
				file->flags |= LFS2_F_ERRED;
				LFS2_TRACE("lfs2_file_write -> %d", err);
				return err;
...
int lfs2_remove(lfs2_t *lfs2, const char *path) {
	...
	err = lfs2_dir_commit(lfs2, &cwd,attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//	err = lfs2_dir_commit(lfs2, &cwd,
//				LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_DELETE, lfs2_tag_id(tag), 0)}
//			));
...
int lfs2_rename(lfs2_t *lfs2, const char *oldpath, const char *newpath) {
	LFS2_TRACE("lfs2_rename(%p, \"%s\", \"%s\")", (void* )lfs2, oldpath,
			newpath);
	...
	if (prevtag == LFS2_ERR_NOENT) {
		...
		lfs2_stag_t res = lfs2_dir_get(lfs2, &newcwd,
				LFS2_MKTAG(0x700, 0x3ff, 0),
	...
	struct lfs2_mattr attrs [] = {
			{LFS2_MKTAG_IF(prevtag != LFS2_ERR_NOENT, LFS2_TYPE_DELETE, newid, 0), NULL},
	...
	err = lfs2_dir_commit(lfs2, &newcwd, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//	err = lfs2_dir_commit(lfs2, &newcwd,
//					LFS2_MKATTRS(
//							{LFS2_MKTAG_IF(prevtag != LFS2_ERR_NOENT, LFS2_TYPE_DELETE, newid, 0)},
//							{LFS2_MKTAG(LFS2_TYPE_CREATE, newid, 0)},
//							{LFS2_MKTAG(lfs2_tag_type3(oldtag), newid, strlen(newpath)), newpath},
//							{LFS2_MKTAG(LFS2_FROM_MOVE, newid, lfs2_tag_id(oldtag)), &oldcwd},
//							{LFS2_MKTAG_IF(samepair, LFS2_TYPE_DELETE, newoldid, 0)}
//						)
//					);
	...
	if (!samepair && lfs2_gstate_hasmove(&lfs2->gstate)) {
		...
		err = lfs2_dir_commit(lfs2, &oldcwd, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//		err = lfs2_dir_commit(lfs2, &oldcwd,
//				LFS2_MKATTRS(
...
lfs2_ssize_t lfs2_getattr(lfs2_t *lfs2, const char *path, uint8_t type,
	...
	tag = lfs2_dir_get(lfs2, &cwd, LFS2_MKTAG(0x7ff, 0x3ff, 0),
			LFS2_MKTAG(LFS2_TYPE_USERATTR + type, id,
					lfs2_min(size, lfs2->attr_max)), buffer);
	if (tag < 0) {
		if (tag == LFS2_ERR_NOENT) {
...
static int lfs2_commitattr(lfs2_t *lfs2, const char *path, uint8_t type,
	...
	return lfs2_dir_commit(lfs2, &cwd, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//	return lfs2_dir_commit(lfs2, &cwd,
...
static int lfs2_init(lfs2_t *lfs2, const struct lfs2_config *cfg) {
	...
	LFS2_ASSERT(
			4 * lfs2_npw2(0xffffffff / (lfs2->cfg->block_size - 2 * 4))
					<= lfs2->cfg->block_size);
	...
	if (lfs2->cfg->read_buffer) {
		lfs2->rcache.buffer = (uint8_t *) lfs2->cfg->read_buffer;
	} else {
		lfs2->rcache.buffer = (uint8_t *) lfs2_malloc(lfs2->cfg->cache_size);
	...
	if (lfs2->cfg->prog_buffer) {
		lfs2->pcache.buffer = (uint8_t *) lfs2->cfg->prog_buffer;
	} else {
		lfs2->pcache.buffer = (uint8_t *) lfs2_malloc(lfs2->cfg->cache_size);
	...
	LFS2_ASSERT(
			lfs2->cfg->lookahead_size % 8 == 0
					&& (uintptr_t )lfs2->cfg->lookahead_buffer % 4 == 0);
	if (lfs2->cfg->lookahead_buffer) {
		lfs2->free.buffer = (uint32_t *) lfs2->cfg->lookahead_buffer;
	} else {
		lfs2->free.buffer = (uint32_t *) lfs2_malloc(lfs2->cfg->lookahead_size);
		if (!lfs2->free.buffer) {
			err = LFS2_ERR_NOMEM;
			goto cleanup;
		}
...
int lfs2_format(lfs2_t *lfs2, const struct lfs2_config *cfg) {
	LFS2_TRACE(
			"lfs2_format(%p, %p {.context=%p, " ".read=%p, .prog=%p, .erase=%p, .sync=%p, " ".read_size=%"PRIu32", .prog_size=%"PRIu32", " ".block_size=%"PRIu32", .block_count=%"PRIu32", " ".block_cycles=%"PRIu32", .cache_size=%"PRIu32", " ".lookahead_size=%"PRIu32", .read_buffer=%p, " ".prog_buffer=%p, .lookahead_buffer=%p, " ".name_max=%"PRIu32", .file_max=%"PRIu32", " ".attr_max=%"PRIu32"})",
	...
	int err = 0;
	{
		...
		struct lfs2_mattr attrs [] = {
				...
				{LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, 0, sizeof(superblock)),	&superblock}
		};
		err = lfs2_dir_commit(lfs2, &root, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//		err = lfs2_dir_commit(lfs2, &root,
...
int lfs2_mount(lfs2_t *lfs2, const struct lfs2_config *cfg) {
	LFS2_TRACE(
			"lfs2_mount(%p, %p {.context=%p, " ".read=%p, .prog=%p, .erase=%p, .sync=%p, " ".read_size=%"PRIu32", .prog_size=%"PRIu32", " ".block_size=%"PRIu32", .block_count=%"PRIu32", " ".block_cycles=%"PRIu32", .cache_size=%"PRIu32", " ".lookahead_size=%"PRIu32", .read_buffer=%p, " ".prog_buffer=%p, .lookahead_buffer=%p, " ".name_max=%"PRIu32", .file_max=%"PRIu32", " ".attr_max=%"PRIu32"})",
			(void* )lfs2, (void* )cfg, cfg->context,
			(void* )(uintptr_t )cfg->read, (void* )(uintptr_t )cfg->prog,
			(void* )(uintptr_t )cfg->erase, (void* )(uintptr_t )cfg->sync,
			cfg->read_size, cfg->prog_size, cfg->block_size, cfg->block_count,
...
//	lfs2_mdir_t dir = { .tail = { 0, 1 } };
	...
	while (!lfs2_pair_isnull(dir.tail)) {
		...
		struct lfs2_dir_find_match data = { lfs2, "littlefs", 8 };
		lfs2_stag_t tag = lfs2_dir_fetchmatch(lfs2,
											  &dir,
											  dir.tail,
											  LFS2_MKTAG(0x7ff, 0x3ff, 0),
...
//	lfs2_mdir_t dir = { .tail = { 0, 1 } };
	...
	while (!lfs2_pair_isnull(dir.tail)) {
		...
		for (uint16_t id = 0; id < dir.count; id++) {
			struct lfs2_ctz ctz;
			lfs2_stag_t tag = lfs2_dir_get(lfs2, &dir,
					LFS2_MKTAG(0x700, 0x3ff, 0),
					LFS2_MKTAG(LFS2_TYPE_STRUCT, id, sizeof(ctz)), &ctz);
			if (tag < 0) {
				if (tag == LFS2_ERR_NOENT) {
			...
			if (lfs2_tag_type3(tag) == LFS2_TYPE_CTZSTRUCT) {
				err = lfs2_ctz_traverse(lfs2, NULL, &lfs2->rcache, ctz.head,
						ctz.size, cb, data);
				if (err) {
					return err;
				}
...
int lfs2_fs_traverse(lfs2_t *lfs2, int (*cb)(void *data, lfs2_block_t block),
		void *data) {
...
static int lfs2_fs_parent_match(void *data, lfs2_tag_t tag, const void *buffer) {
	struct lfs2_fs_parent_match *find = (struct lfs2_fs_parent_match *) data;
	lfs2_t *lfs2 = find->lfs2;
	const struct lfs2_diskoff *disk = (const struct lfs2_diskoff *) buffer;
	(void) tag;

	lfs2_block_t child[2];
	int err = lfs2_bd_read(lfs2, &lfs2->pcache, &lfs2->rcache,
			lfs2->cfg->block_size, disk->block, disk->off, &child,
			sizeof(child));
	if (err) {
		return err;
	}
...
static int lfs2_fs_relocate(lfs2_t *lfs2, const lfs2_block_t oldpair[2],
	...
	if (tag != LFS2_ERR_NOENT) {
		...
		if (lfs2_gstate_hasmovehere(&lfs2->gstate, parent.pair)) {
			...
			LFS2_DEBUG(
					"Fixing move while relocating " "{0x%"PRIx32", 0x%"PRIx32"} 0x%"PRIx16"\n",
	...
	if (err != LFS2_ERR_NOENT) {
		...
		err = lfs2_dir_commit(lfs2, &parent, attrs,  sizeof(attrs) / sizeof(struct lfs2_mattr) );
//		err = lfs2_dir_commit(lfs2, &parent,
//						LFS2_MKATTRS(
//								{LFS2_MKTAG_IF(moveid != 0x3ff, LFS2_TYPE_DELETE, moveid, 0)},
//								{LFS2_MKTAG(LFS2_TYPE_TAIL + parent.split, 0x3ff, 8), newpair}
//						));
...
static int lfs2_fs_demove(lfs2_t *lfs2) {
	...
	err = lfs2_dir_commit(lfs2, &movedir, attrs, sizeof(attrs) / sizeof(struct lfs2_mattr) );
//	err = lfs2_dir_commit(lfs2, &movedir,
//			LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_DELETE, moveid, 0)}));
	...
	while (!lfs2_pair_isnull(pdir.tail)) {
		...
		if (!pdir.split) {
			...
			if (!lfs2_pair_sync(pair, pdir.tail)) {
				// we have desynced
				LFS2_DEBUG(
						"Fixing half-orphan {0x%"PRIx32", 0x%"PRIx32"} " "-> {0x%"PRIx32", 0x%"PRIx32"}",
				...
				err = lfs2_dir_commit(lfs2, &pdir, attrs,  sizeof(attrs) / sizeof(struct lfs2_mattr) );
//				err = lfs2_dir_commit(lfs2, &pdir,
//						LFS2_MKATTRS(
//								{LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8),	pair}
//						));
				lfs2_pair_fromle32(pair);
